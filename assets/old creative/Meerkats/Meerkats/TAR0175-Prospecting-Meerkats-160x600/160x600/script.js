...
function frame1(){
	...
	.set( [t1a,t1b, t1c, t1d, t2a,t2b,t2c], { y: 10 })
	.set( [end_logo, end_txt, cta ], { y: 20 })
	
	
	
	.add('start', 0)
	.from( bg_img, 9, { y:0, x:0, scale:1, transformOrigin:'0% 0%'  }, "start")
	.from( bg_leaf1, 2, { y:50, x:-180, rotation:60, ease:Power2.easeOut, transformOrigin:'0% 100%' }, "start+=0.5")
	...
	.to( t1c, 1, { alpha: 1, y:0 }, "t1+=0.5")
	.to( t1d, 1, { alpha: 1, y:0 }, "t1+=0.75")
		
		
	.add( "t2", "t1+=3" )
	.to( t1a, 1, { alpha: 0, y:animOffsetDist }, "t2")
	.to( t1b, 1, { alpha: 0, y:animOffsetDist }, "t2")
	.to( t1c, 1, { alpha: 0, y:animOffsetDist }, "t2")
	.to( t1d, 1, { alpha: 0, y:animOffsetDist }, "t2")
	.to( t2a, 1, { alpha: 1, y:0 }, "t2+=0.5")
	.to( t2b, 1, { alpha: 1, y:0}, "t2+=0.75")
	.to( t2c, 1, { alpha: 1, y:0 }, "t2+=1")
	
	
	.add( "endFade", "t2+=3.5" )
	.to( [t2a,t2b,t2c], 1, { alpha: 0, y:animOffsetDist }, "endFade")
